...
pub contract TouchstoneSearchingforGod: NonFungibleToken {
	...
	pub struct NFTMetadata {
		...
		init(_name: String, _description: String, _image: MetadataViews.IPFSFile, _thumbnail: MetadataViews.IPFSFile?, _price: UFix64?, _extra: {String: AnyStruct}) {
			self.metadataId = TouchstoneSearchingforGod.nextMetadataId
			self.name = _name
			...
			TouchstoneSearchingforGod.nextMetadataId = TouchstoneSearchingforGod.nextMetadataId + 1
		}
	}
	...
	pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver {
		...
		pub fun getMetadata(): NFTMetadata {
			return TouchstoneSearchingforGod.getNFTMetadata(self.metadataId)!
		}
			...
			switch view {
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(
						storagePath: TouchstoneSearchingforGod.CollectionStoragePath,
						publicPath: TouchstoneSearchingforGod.CollectionPublicPath,
						providerPath: TouchstoneSearchingforGod.CollectionPrivatePath,
						...
						createEmptyCollectionFunction: (fun (): @NonFungibleToken.Collection {
								return <- TouchstoneSearchingforGod.createEmptyCollection()
						})
					)
				case Type<MetadataViews.ExternalURL>():
          return MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/TouchstoneSearchingforGod"))
				case Type<MetadataViews.NFTCollectionDisplay>():
					let squareMedia = MetadataViews.Media(
						file: TouchstoneSearchingforGod.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile,
						mediaType: "image"
					)
					...
					if let bannerImage = TouchstoneSearchingforGod.getOptionalCollectionAttribute(key: "bannerImage") as! MetadataViews.IPFSFile? {
					...
					return MetadataViews.NFTCollectionDisplay(
						name: TouchstoneSearchingforGod.getCollectionAttribute(key: "name") as! String,
						description: TouchstoneSearchingforGod.getCollectionAttribute(key: "description") as! String,
						externalURL: MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/TouchstoneSearchingforGod")),
						squareImage: squareMedia,
						bannerImage: bannerMedia ?? squareMedia,
						socials: TouchstoneSearchingforGod.getCollectionAttribute(key: "socials") as! {String: MetadataViews.ExternalURL}
					)
		...
		init(_metadataId: UInt64, _recipient: Address) {
			pre {
				TouchstoneSearchingforGod.metadatas[_metadataId] != nil:
					"This NFT does not exist yet."
				!TouchstoneSearchingforGod.primaryBuyers.containsKey(_metadataId):
					"This NFT has already been minted."
			}
			self.id = self.uuid
			self.metadataId = _metadataId

			TouchstoneSearchingforGod.primaryBuyers[_metadataId] = _recipient
			TouchstoneSearchingforGod.totalSupply = TouchstoneSearchingforGod.totalSupply + 1
	...
	pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection {
		...
		pub fun claim() {
			if let storage = &TouchstoneSearchingforGod.nftStorage[self.owner!.address] as &{UInt64: NFT}? {
	...
	pub fun mintNFT(metadataId: UInt64, recipient: &{NonFungibleToken.Receiver}, payment: @FlowToken.Vault): UInt64 {
		...
		if let royalty = TouchstoneSearchingforGod.getOptionalCollectionAttribute(key: "royalty") as! MetadataViews.Royalty? {
	...
	pub resource Administrator {
		pub fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra: {String: AnyStruct}) {
			TouchstoneSearchingforGod.metadatas[TouchstoneSearchingforGod.nextMetadataId] = NFTMetadata(
				_name: name,
		...
		pub fun mintNFT(metadataId: UInt64, recipient: Address) {
			pre {
				EmeraldPass.isActive(user: TouchstoneSearchingforGod.account.address): "You must have an active Emerald Pass subscription to airdrop NFTs. You can purchase Emerald Pass at https://pass.ecdao.org/"
			}
			let nft <- create NFT(_metadataId: metadataId, _recipient: recipient)
			if let recipientCollection = getAccount(recipient).getCapability(TouchstoneSearchingforGod.CollectionPublicPath).borrow<&TouchstoneSearchingforGod.Collection{NonFungibleToken.CollectionPublic}>() {
				recipientCollection.deposit(token: <- nft)
			} else {
				if let storage = &TouchstoneSearchingforGod.nftStorage[recipient] as &{UInt64: NFT}? {
					storage[nft.id] <-! nft
				} else {
					TouchstoneSearchingforGod.nftStorage[recipient] <-! {nft.id: <- nft}
				}
			}
		}
		...
		pub fun changeField(key: String, value: AnyStruct) {
			TouchstoneSearchingforGod.collectionInfo[key] = value
		}
	}
	...
	pub fun getPriceOfNFT(_ metadataId: UInt64): UFix64? {
		if let metadata: TouchstoneSearchingforGod.NFTMetadata = self.getNFTMetadata(metadataId) {
	...
	pub fun getClaimableNFTs(user: Address): {UInt64: NFTMetadata} {
		...
		if let storage = &TouchstoneSearchingforGod.nftStorage[user] as &{UInt64: NFT}? {
			for id in storage.keys {
	...
	init(
		...
		self.CollectionStoragePath = /storage/TouchstoneSearchingforGodCollection_0x903a64bd3777b67a
		self.CollectionPublicPath = /public/TouchstoneSearchingforGodCollection_0x903a64bd3777b67a
		self.CollectionPrivatePath = /private/TouchstoneSearchingforGodCollection_0x903a64bd3777b67a
		self.AdministratorStoragePath = /storage/TouchstoneSearchingforGodAdministrator_0x903a64bd3777b67a
