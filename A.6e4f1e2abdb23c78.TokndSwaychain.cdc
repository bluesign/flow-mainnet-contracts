...
This is the contract for TokndSwaychain NFTs! 
...
pub contract TokndSwaychain: NonFungibleToken {
    ...
    NiftoryNonFungibleToken.NFTPublic
  {
    ...
    pub fun contract(): &{NiftoryNonFungibleToken.ManagerPublic} {
      return TokndSwaychain.contract()
    }
    ...
    init(setId: Int, templateId: Int, serial: UInt64) {
      self.id = TokndSwaychain.totalSupply
      TokndSwaychain.totalSupply =
        TokndSwaychain.totalSupply + 1
      self.setId = setId
    ...
    NiftoryNonFungibleToken.CollectionPrivate
  {
    ...
    pub fun deposit(token: @NonFungibleToken.NFT) {
      let token <- token as! @TokndSwaychain.NFT
    ...
    pub fun depositBulk(tokens: @[NonFungibleToken.NFT]) {
      while tokens.length > 0 {
        let token <- tokens.removeLast() as! @TokndSwaychain.NFT
    ...
    NiftoryNonFungibleToken.ManagerPrivate 
  {
    ...
    pub fun metadata(): AnyStruct? {
      return TokndSwaychain.metadata
    }
      ...
      &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public}
    {
      return NiftoryNFTRegistry
        .getSetManagerPublic(
          TokndSwaychain.REGISTRY_ADDRESS,
          TokndSwaychain.REGISTRY_BRAND
        )
    }
      ...
      return NiftoryNFTRegistry
        .buildNFTCollectionData(
          ...
          (fun (): @NonFungibleToken.Collection {
            return <-TokndSwaychain.createEmptyCollection()
          })
        )
    }
    ...
    pub fun modifyContractMetadata(): auth &AnyStruct {
      return &TokndSwaychain.metadata as auth &AnyStruct
    }

    pub fun replaceContractMetadata(_ metadata: AnyStruct?) {
      TokndSwaychain.metadata = metadata
    }
  ...
  init(
    ...
    let record = NiftoryNFTRegistry.generateRecord(
      ...
      project: "clbh2fkuw0000jy0gu9nmh7e1_TokndSwaychain"
    )

    self.REGISTRY_ADDRESS = 0x32d62d5c43ad1038
    self.REGISTRY_BRAND = "clbh2fkuw0000jy0gu9nmh7e1_TokndSwaychain"
    ...
    self
      ...
      .save<@MutableMetadataSetManager.Manager>(
        <-MutableMetadataSetManager.create(
          name: "TokndSwaychain",
          description: "The set manager for TokndSwaychain."
        ),
    ...
    let contractName = "TokndSwaychain"
