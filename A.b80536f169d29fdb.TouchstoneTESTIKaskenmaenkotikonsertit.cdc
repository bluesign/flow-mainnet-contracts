...
pub contract TouchstoneTESTIKaskenmaenkotikonsertit: NonFungibleToken {
	...
	pub struct NFTMetadata {
		...
		init(_name: String, _description: String, _image: MetadataViews.IPFSFile, _thumbnail: MetadataViews.IPFSFile?, _price: UFix64?, _extra: {String: AnyStruct}, _supply: UInt64) {
			self.metadataId = TouchstoneTESTIKaskenmaenkotikonsertit.nextMetadataId
			self.name = _name
	...
	pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver {
		...
		pub fun getMetadata(): NFTMetadata {
			return TouchstoneTESTIKaskenmaenkotikonsertit.getNFTMetadata(self.metadataId)!
		}
			...
			switch view {
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(
						storagePath: TouchstoneTESTIKaskenmaenkotikonsertit.CollectionStoragePath,
						publicPath: TouchstoneTESTIKaskenmaenkotikonsertit.CollectionPublicPath,
						providerPath: TouchstoneTESTIKaskenmaenkotikonsertit.CollectionPrivatePath,
						...
						createEmptyCollectionFunction: (fun (): @NonFungibleToken.Collection {
								return <- TouchstoneTESTIKaskenmaenkotikonsertit.createEmptyCollection()
						})
					)
				case Type<MetadataViews.ExternalURL>():
          return MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/TouchstoneTESTIKaskenmaenkotikonsertit"))
				case Type<MetadataViews.NFTCollectionDisplay>():
					let squareMedia = MetadataViews.Media(
						file: TouchstoneTESTIKaskenmaenkotikonsertit.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile,
						mediaType: "image"
					)
					...
					if let bannerImage = TouchstoneTESTIKaskenmaenkotikonsertit.getOptionalCollectionAttribute(key: "bannerImage") as! MetadataViews.IPFSFile? {
					...
					return MetadataViews.NFTCollectionDisplay(
						name: TouchstoneTESTIKaskenmaenkotikonsertit.getCollectionAttribute(key: "name") as! String,
						description: TouchstoneTESTIKaskenmaenkotikonsertit.getCollectionAttribute(key: "description") as! String,
						externalURL: MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/TouchstoneTESTIKaskenmaenkotikonsertit")),
						squareImage: squareMedia,
						bannerImage: bannerMedia ?? squareMedia,
						socials: TouchstoneTESTIKaskenmaenkotikonsertit.getCollectionAttribute(key: "socials") as! {String: MetadataViews.ExternalURL}
					)
		...
		init(_metadataId: UInt64, _serial: UInt64, _recipient: Address) {
			pre {
				TouchstoneTESTIKaskenmaenkotikonsertit.metadatas[_metadataId] != nil:
					"This NFT does not exist yet."
				_serial < TouchstoneTESTIKaskenmaenkotikonsertit.getNFTMetadata(_metadataId)!.supply:
					"This serial does not exist for this metadataId."
				!TouchstoneTESTIKaskenmaenkotikonsertit.getNFTMetadata(_metadataId)!.purchasers.containsKey(_serial):
			...
			if let buyersRef = &TouchstoneTESTIKaskenmaenkotikonsertit.primaryBuyers[_recipient] as &{UInt64: [UInt64]}? {
				...
				TouchstoneTESTIKaskenmaenkotikonsertit.primaryBuyers[_recipient] = {_metadataId: [_serial]}
			}

			// Update who bought this serial inside NFTMetadata so it cannot be purchased again.
			let metadataRef = (&TouchstoneTESTIKaskenmaenkotikonsertit.metadatas[_metadataId] as &NFTMetadata?)!
			metadataRef.purchased(serial: _serial, buyer: _recipient)

			TouchstoneTESTIKaskenmaenkotikonsertit.totalSupply = TouchstoneTESTIKaskenmaenkotikonsertit.totalSupply + 1
	...
	pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection {
		...
		pub fun claim() {
			if let storage = &TouchstoneTESTIKaskenmaenkotikonsertit.nftStorage[self.owner!.address] as &{UInt64: NFT}? {
	...
	pub fun mintNFT(metadataId: UInt64, recipient: &{NonFungibleToken.Receiver}, payment: @FlowToken.Vault, serial: UInt64): UInt64 {
		...
		if let royalty = TouchstoneTESTIKaskenmaenkotikonsertit.getOptionalCollectionAttribute(key: "royalty") as! MetadataViews.Royalty? {
	...
	pub resource Administrator {
		pub fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra: {String: AnyStruct}, supply: UInt64) {
			TouchstoneTESTIKaskenmaenkotikonsertit.metadatas[TouchstoneTESTIKaskenmaenkotikonsertit.nextMetadataId] = NFTMetadata(
				_name: name,
			...
			TouchstoneTESTIKaskenmaenkotikonsertit.nextMetadataId = TouchstoneTESTIKaskenmaenkotikonsertit.nextMetadataId + 1
		}
		...
		pub fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address) {
			pre {
				EmeraldPass.isActive(user: TouchstoneTESTIKaskenmaenkotikonsertit.account.address): "You must have an active Emerald Pass subscription to airdrop NFTs. You can purchase Emerald Pass at https://pass.ecdao.org/"
			}
			let nft <- create NFT(_metadataId: metadataId, _serial: serial, _recipient: recipient)
			if let recipientCollection = getAccount(recipient).getCapability(TouchstoneTESTIKaskenmaenkotikonsertit.CollectionPublicPath).borrow<&TouchstoneTESTIKaskenmaenkotikonsertit.Collection{NonFungibleToken.CollectionPublic}>() {
				recipientCollection.deposit(token: <- nft)
			} else {
				if let storage = &TouchstoneTESTIKaskenmaenkotikonsertit.nftStorage[recipient] as &{UInt64: NFT}? {
					storage[nft.id] <-! nft
				} else {
					TouchstoneTESTIKaskenmaenkotikonsertit.nftStorage[recipient] <-! {nft.id: <- nft}
				}
			}
		}
		...
		pub fun changeField(key: String, value: AnyStruct) {
			TouchstoneTESTIKaskenmaenkotikonsertit.collectionInfo[key] = value
		}
	}
	...
	pub fun getPriceOfNFT(_ metadataId: UInt64): UFix64? {
		if let metadata: TouchstoneTESTIKaskenmaenkotikonsertit.NFTMetadata = self.getNFTMetadata(metadataId) {
	...
	pub fun getClaimableNFTs(user: Address): {UInt64: NFTMetadata} {
		...
		if let storage = &TouchstoneTESTIKaskenmaenkotikonsertit.nftStorage[user] as &{UInt64: NFT}? {
			for id in storage.keys {
	...
	init(
		...
		self.CollectionStoragePath = /storage/TouchstoneTESTIKaskenmaenkotikonsertitCollection_0xb80536f169d29fdb
		self.CollectionPublicPath = /public/TouchstoneTESTIKaskenmaenkotikonsertitCollection_0xb80536f169d29fdb
		self.CollectionPrivatePath = /private/TouchstoneTESTIKaskenmaenkotikonsertitCollection_0xb80536f169d29fdb
		self.AdministratorStoragePath = /storage/TouchstoneTESTIKaskenmaenkotikonsertitAdministrator_0xb80536f169d29fdb
